...
- **Client-side Routing**: Implemented using `pushState` for SEO-friendly URLs (`/`, `/deal-analyser`, `/sdlt-calculator`) with Express fallback routes. Unique title, description, canonical, OG, and Twitter metadata per route. Body class system: `deal-mode` (Deal Calculator, homepage and `/deal-analyser`), `sdlt-mode` (SDLT calculator) controls route-specific visibility. Legacy `/simple-analyser` maps to Deal Calculator mode.
- **Route-Specific Underfold Content**: Each route has its own below-the-fold landing content (intro, info cards, FAQs) controlled via CSS body classes. Deal Calculator has 3-column card grid on desktop. All sections support mobile toggle (collapsible) pattern.
...
- **Deal Rating**: A–F grades based on fixed Net Yield (Asset) thresholds: A≥8%, B 7–7.99%, C 6–6.99%, D 5–5.99%, F<5%. Grade labels: A=Excellent, B=Strong, C=Moderate, D=Below Target, E=Poor, F=Very Weak. Colours: A/B green, C amber, D orange, F red. Independent of target yield.
...
- **History Sort Toggle**: Pill-style "Latest / Best Yield" toggle in history section. Default: Latest (newest first by timestamp). Persisted in localStorage (`historySortMode`).
- **ROI Rank Toggle**: Toggle switch near history heading. Only visible in "Best Yield" sort mode — hidden and unchecked in "Latest" mode (ranking is meaningless for chronological order). When ON, shows rank badges (#1, #2, #3...) on history rows based on Net Yield descending sort. Compare modal always shows ranks with gold/silver/bronze badges.
...
- **Re-analyse Button**: Appears next to Target Yield input when the value changes from what was last analysed (or from default 7%). Clicking re-runs the deal analysis without scrolling to Analyse Deal. Hidden in SDLT Calculator Only mode. Resets on mode switch and Start Again. Uses `lastAnalysedTargetYield` and `hasAnalysedOnce` state tracking. `getCurrentTargetYield()` is the single source of truth for target yield.
